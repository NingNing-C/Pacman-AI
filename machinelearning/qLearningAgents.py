...
class PacmanDeepQAgent(PacmanQAgent):
    ...
    def compute_q_targets(self, minibatch, network = None, target_network=None, doubleQ=False):
        ...
        Q_next_s=network.run(next_states).data
        Q_t_next_s = target_network.run(next_states).data
        index = np.argmax(Q_next_s,axis=1)
        #print("Q_t_nest_s",Q_t_next_s.data)
        #print("index",index)
        #Q=np.array([self.getQValue(state,action) for state,action in zip(states_np,actions)])
        #print(states_np[0])
        bonus = np.array([1/(2*np.sqrt((self.counts[state[0],state[1]])/100)) for state in states_np])
        #print('N',self.counts[states_np[0][0],states_np[0][1]])
        #print("rewards",rewards[0])
        #print("t",Q_t_next_s[0,index[0]])
        #print("done",dones[0])
        #print("Q_t_next_s",Q_t_next_s.data)
        #print("bouns",bonus)
        #print ("actions",actions)
        new_q = rewards+(1-dones)*self.discount*Q_t_next_s[np.arange(Q_t_next_s.shape[0]),index]+bonus
        Q_target = np.copy(Q_predict)
        
        Q_target[np.arange(Q_target.shape[0]),actions] = new_q
        "*** YOUR CODE STARTS HERE ***"
