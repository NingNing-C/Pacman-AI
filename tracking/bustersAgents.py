    ...
    def chooseAction(self, gameState):
        ...
        for action in legal:
            ...
            for beliefs in livingGhostPositionDistributions:
                ghost_p=beliefs.argMax()
                mazeDis=self.distancer.getDistance(ghost_p,successorPosition)
                if action not in mazeDis_action.keys():
                    mazeDis_action[action]=mazeDis
                else:
                    if mazeDis_action[action]>mazeDis:
                        mazeDis_action[action]=mazeDis
        return min(mazeDis_action,key=mazeDis_action.get)
